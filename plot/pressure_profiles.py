...
class PlotPressureProfiles:
    ...
    def plot_pressure_profile(self, model_names=None, r_range=(1, 2000), n_points=200, 
        ...
        """
        Plot radial pressure profiles for cluster models using stored model data.
        Automatically plots all available models, or specified models.
        """
        ...
        for model_name in model_names:
            ...
            # Build transform input (gets r_s via R500/c500 for A10; otherwise leaves base)
            xform = TransformInput(model_params, model_type)
            input_par = xform.run()

            # Build normalized radius for model evaluation
            if model_type == 'A10Pressure' and mass is not None:
                # Use r_s from transform (in Mpc)
                r_s_kpc = input_par['major'] * 1000.0
                r_norm = r_kpc / r_s_kpc
            else:
                # For non-A10 or missing mass, assume r_kpc is already the evaluation variable
                # Here we choose to normalize by 1 kpc to avoid changing shapes; users can
                # interpret units accordingly for quick checks.
                r_norm = r_kpc / 1.0

            # Choose rs mesh as in Veszee
            ...
            # Evaluate model shape using models.py functions
            if model_type == 'A10Pressure':
                ...
                alpha = model_params.get('alpha', 1.05)
                beta = model_params.get('beta', 5.49)
                gamma = model_params.get('gamma', 0.31)
                e = model_params.get('e', model_params.get('eccentricity', 0.0))
                shape = gnfwProfile(rs, 0.0, 1.0, 1.0, e, alpha, beta, gamma)
            elif model_type == 'betaPressure':
                beta = model_params.get('beta', 0.7)
                e = model_params.get('e', model_params.get('eccentricity', 0.0))
                shape = betaProfile(rs, 0.0, 1.0, 1.0, e, beta)
            else:
                raise ValueError(f"Unknown pressure profile type: {model_type}")

            # Interp to r_norm grid
            pressure_norm = np.interp(r_norm, rs, shape)

            # Convert to physical units (keV/cm^3) if mass & z present (A10 case)
            if mass is not None:
        ...
        ax.set_ylabel('Pressure (keV cm⁻³)' if any(
            model_info['parameters']['model'].get('mass') is not None for model_info in self.models.values()
